"""LIT-5836 regression: a single cluster node's ConnectionError/TimeoutError must reset
only that node's connections, not tear down the whole cluster client for every other
concurrent caller. Live confirmation against a real 3-master local cluster (pausing one
node with CLIENT PAUSE) showed 100% of concurrent commands to the other two, untouched
nodes stalling for the full pause duration before this fix, and zero after -- these tests
pin the same behavior at the unit level so it can run without a live Redis Cluster."""
...
class _FakeClusterNode:
    def __init__(self, name: str, raises: Exception | None = None, response: Any = None) -> None:
        self.name = name
        self.execute_command = AsyncMock(side_effect=raises, return_value=response)
        self.disconnect = AsyncMock()


def _build_cluster_instance() -> Any:
...
async def test_node_level_error_resets_only_that_node_not_the_whole_client(error_cls):
...
@pytest.mark.asyncio
async def test_successful_command_touches_neither_disconnect_nor_aclose():
...
async def test_busy_loading_and_max_connections_reraise_without_any_reset(error_cls):
...
@pytest.mark.asyncio
async def test_cluster_down_error_still_triggers_a_full_reinit():
...
@pytest.mark.asyncio
async def test_moved_error_still_triggers_reinit_after_reinitialize_steps():
    ...
    class _FakeNodesManager:
        ...
        def get_node_from_slot(self, *_args: Any, **_kwargs: Any) -> Any:
            return target_node

    instance.nodes_manager = _FakeNodesManager()
