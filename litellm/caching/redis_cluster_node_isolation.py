...
def get_litellm_async_redis_cluster_class() -> type["_AsyncRedisClusterType"]:
    ...
    if redis.__version__ not in _VERIFIED_REDIS_VERSIONS:
        verbose_logger.warning(
            "LIT-5836: redis-py %s is not in the set this cluster-teardown-storm fix was verified "
            ...
            **kwargs: object,  # kwargs-ok: overrides redis-py's own **kwargs signature; the keyword contract is defined by the Redis command being dispatched, not by this method
            ...
            while ttl > 0:
                ...
                try:
                    ...
                    # LIT-5836: reset only the node that actually failed instead of the
                    # upstream default (`await self.aclose()`, a full-cluster teardown that
                    # forces every other concurrent caller through the shared reinit lock).
